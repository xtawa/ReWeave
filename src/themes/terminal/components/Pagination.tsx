...
export function Pagination({ current, total, baseUrl }: PaginationProps) {
    ...
    return (
        <nav class="mt-10 flex items-center justify-between gap-4 bg-black/40 border border-emerald-500/20 rounded-none p-3" aria-label="Pagination">
            {prevPage ? (
                <a href={prevPage} class="border border-emerald-500/40 px-3 py-1 text-zinc-200 no-underline hover:text-emerald-300 hover:border-emerald-400/70">{'< prev'}</a>
            ) : (
            ...
            {nextPage ? (
                <a href={nextPage} class="border border-emerald-500/40 px-3 py-1 text-zinc-200 no-underline hover:text-emerald-300 hover:border-emerald-400/70">{'next >'}</a>
            ) : (
