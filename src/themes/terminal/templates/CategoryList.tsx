            ...
            <main class="bg-black/40 border border-emerald-500/20 p-5">
                ...
                <div class="space-y-2">
                    {Array.from(categories.entries()).map(([category, posts]) => (
                        <a href={`/categories/${safeSlug(category)}`} class="flex items-center justify-between border border-emerald-500/30 px-3 py-2 text-zinc-300 no-underline hover:border-emerald-400/70 hover:text-emerald-300">
                            <span>~/{category}</span>
