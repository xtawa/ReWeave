        ...
        <Layout title={t('archive', config.language)} url="/archive">
            ...
            <main class="bg-black/40 border border-emerald-500/20 p-5 space-y-6">
                ...
                <div class="flex flex-wrap gap-3">
                    <a href="/categories" class="border border-emerald-500/30 px-3 py-1 text-zinc-300 no-underline hover:border-cyan-400">categories: {categoriesCount}</a>
                    <a href="/tags" class="border border-emerald-500/30 px-3 py-1 text-zinc-300 no-underline hover:border-cyan-400">tags: {tagsCount}</a>
                </div>
                {years.map((year) => (
                    <section>
                        ...
                        <ul class="space-y-2">
                            {postsByYear.get(year)!.map((post) => (
                                <li>
                                    <a href={`/posts/${safeSlug(post.abbrlink || post.slug)}`} class="text-zinc-300 no-underline border-b border-transparent hover:border-cyan-400 hover:text-cyan-300">
